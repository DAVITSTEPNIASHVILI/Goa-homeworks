...
export const signUp = catchAsync(async (req, res, next) => {
    ...
    const url = `${url.protocol}::/${url.domain}/api/auth/${code}`
    const html = `
        <!DOCTYPE html>
        <html lang="en">
        <head>
...
export const verify = catchAsync(async (req, res, next) => {
    ...
    const foundUser = await Users.findOne({verificationCode: code})

    if (!foundUser) {
        return new AppError("Invalid verification code!", 400)
    }
